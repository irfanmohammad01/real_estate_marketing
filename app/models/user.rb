class User < ApplicationRecord
  ...
  def superuser?
    role.name == Role::ROLES[:superuser]
  end


  validates :email, presence: true, uniqueness: { scope: :organization_id }, format: { with: URI::MailTo::EMAIL_REGEXP, message: "must be a valid email address" }
