class UserMailer < ApplicationMailer
  def invitation_email(user, invitation_link, temporary_password)
    system_org = Organization.find_by(is_system: true)

    template = EmailTemplate
      .joins(:email_type, :organization)
      .where(
        email_types: { key: "INVITE_EMAIL" },
        organizations: { id: system_org.id }
      )
      .find_by(name: template_name_for(user))

    raise "Email template not found for INVITE_EMAIL in system organization" unless template
  ...
  def template_name_for(user)
    if user.role.name == ENV["ORG_ADMIN_ROLE"]
      "Admin Invitation Mail"
    else
      "Agent Invitation Mail"
    end
  end
